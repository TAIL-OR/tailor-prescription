...
class Data:
  def __init__(self, filename):
    with open(filename) as file_object:
      self.F = range(int(file_object.readline())) # F: set of facilities
      n_req = 0
      req = []
      for n in file_object.readline().split():
        n_req += int(n)
        req.append(range(n_req))
      ...
      self.d = int(file_object.read_line()) # d: demand of ICU beds
      ...
      self.n = [int(n) for n in file_object.readline().split()] # n: necessary rate of each requirement per ICU bed
      ...
      for _ in np.concatenate(self.E, self.S):
        costs_for_req_j = []
        for _ in self.F:
          self.costs_for_req_j.append([int(n) for n in file_object.readline().split()])
...
class Model:
  def __init__(self, data):
    ...
    model.z = pyo.Var(model.F, np.concatenate(model.E, model.I, model.S), within=pyo.Integers) # z: number of each requirement acquired by each facility
    model.w = pyo.Var(model.F, np.concatenate(model.E, model.I), within=pyo.Integers) # w: number of each requirement repaired in each facility
    model.v = pyo.Var(np.concatenate(model.E, model.S), model.F, model.F, within=pyo.Integers) # v: number of each requirement transferred from each facility to each facility
    # Objective function
    model.objective = pyo.Objective(expr=sum(data.c[i]*model.y[i] + sum(data.p[j]*model.z[i, j]
      for j in np.concatenate(model.E, model.I, model.S)) + sum(data.m[j]*model.w[i, j]
      for j in np.concatenate(model.E, model.I)) + sum(sum(data.t[j][i][l]*model.v[j, i, l]
      for l in model.F if l != i) for j in np.concatenate(model.E, model.S)) for i in model.F),
      sense=pyo.minimize)
    # Constraints
    ...
    for i in model.F:
      for j in np.concatenate(model.E, model.I):
